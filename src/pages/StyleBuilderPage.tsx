...
function FeatureCard({ children, accent = GOLD }: { children: React.ReactNode; accent?: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: 'easeOut' }}
      style={{ width: '100%', background: 'rgba(22, 12, 3, 0.4)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: `1px solid ${BORDER}`, borderRadius: 12, overflow: 'hidden', marginTop: 10, borderTop: `2px solid ${accent}`, boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
      {children}
    </motion.div>
  );
}
