  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const { data: avail } = await supabase
        .from('orders')
        .select('id, user_id, status, total_amount, payment_method, coupon_code, discount_amount, created_at, delivery_pin, driver_id, pin_verified, pin_attempts, claimed_at, delivered_at, address, order_items(id, order_id, product_id, color_name, size, quantity, price)')
        .eq('status', 'packed')
        .is('driver_id', null)
